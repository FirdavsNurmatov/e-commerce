...
export const createCategory = async ({
    ...
    if (!created_at || !updated_at) {
        var query =
            'Insert into category(name,description,tag) Values($1,$2,$3)'
        var arr = [name, description, tag]
    } else {
        var query =
            'Insert into category(name,description,tag,created_at,updated_at) Values($1,$2,$3,$4,$5)'
...
export const updatecategory = async ({
    ...
    if (result.rows.length >= 1) {
        if (!created_at || !updated_at) {
            ...
            var updatequery =
                'Update category set name=$1,description=$2,tag=$3,created_at=$4,updated_at=$5 where id=$6'
